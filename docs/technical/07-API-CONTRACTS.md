...
*Every NestJS endpoint. Schemas are zod, defined once in `packages/shared/src/contracts/` and imported by both apps — this doc mirrors them. Base path `/v1`. All non-webhook routes require `Authorization: Bearer <supabase JWT>` (03 §3). POSTs accept an `Idempotency-Key` header.*
...
*(Alternative considered: direct Supabase update — rejected: keeping one candidate archives its siblings atomically.)*
