...
| **Never-Include**          | no `never_include.term` appears (word-boundary match) — post-generation re-check even though prompt excluded it; leak = P1                                     | regenerate; second fail → hard fail + PostHog alert |
