...
*Supabase PostgreSQL. Implements product docs 05 (scope), 09 (features), 10 (memory). RLS is the primary security boundary (see 14). All tables live in schema `public`; all user tables carry `user_id uuid references auth.users(id) on delete cascade`.*
